...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
			...
			//map = new UnfoldingMap(this, 100, 0, 900, 500*2, new Microsoft.HybridProvider());
	    ...
	    for(PointFeature feature : earthquakes) {
		  //check if LandQuake
		  if(isLand(feature)) {
			LandQuakeMarker l = new LandQuakeMarker(feature);
		    quakeMarkers.add(l);							//for debugging to see whats in there
		  }
		  // OceanQuakes
		  else {
	   ...
	   Location dublinLocation = new Location(59.329, 18.069);
...
//	   SimplePointMarker berlinMarker = new SimplePointMarker(berlinLocation);
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			if(isInCountry(earthquake,m)){
				//System.out.println("incountry " + m.getLocation().getLat()+ m.getLocation().getLon());	
	...
	private void printQuakes() {
		...
		int oCount = 0;
		for (Marker cm : countryMarkers) {
			//System.out.println("hoho " + cm.getId());
			int qCount = 0;
			String theCountry=(String) cm.getProperty("name");
			for (Marker qm : quakeMarkers) {
				if (qm instanceof LandQuakeMarker) {							
					String incountry=(String) qm.getProperty("country");
					if (incountry ==theCountry ) {
						qCount++;
					}
				}
		...
		if(country.getClass() == MultiMarker.class) {
			...
			for(Marker marker : ((MultiMarker)country).getMarkers()) {
				...
				if(((AbstractShapeMarker)marker).isInsideByLocation(checkLoc)) {
					earthquake.addProperty("country", country.getProperty("name"));  //ADDS country here
